...
const HUB = path.join(here, 'hub.mjs');
const STUB = path.join(here, 'stub-adapter.mjs');
...
test('/goal wall-clock cap binds (dispatch maxMs, clamped)', async () => {
  const { dispatch, LOOP_MAX_MS } = await import('./commands.mjs');
  const events = [];
...
test('goalMet: last marker wins and NOT MET never reads as met', async () => {
  const { goalMet } = await import('./commands.mjs');
